...
std::vector<STrack> BYTETracker::update(const ai_msgs::msg::PerceptionTargets::SharedPtr msg,
    ...
    if (msg->targets.size() > 0) {
        for (int i = 0; i < msg->targets.size(); i++) {
            ...
            tlbr_.resize(4);
            tlbr_[0] = msg->targets[i].rois[0].rect.x_offset;
            tlbr_[1] = msg->targets[i].rois[0].rect.y_offset;
            tlbr_[2] = msg->targets[i].rois[0].rect.x_offset + msg->targets[i].rois[0].rect.width;
            tlbr_[3] = msg->targets[i].rois[0].rect.y_offset + msg->targets[i].rois[0].rect.height;

            float score = msg->targets[i].rois[0].confidence;

            STrack strack(STrack::tlbr_to_tlwh(tlbr_),
                          score,
                          cv::Scalar(255, 112, 112),
                          msg->targets[i].type);
    ...
    u_detection.clear();
    linear_assignment(
        dists, dist_size, dist_size_size, 0.5, matches, u_track, u_detection);
