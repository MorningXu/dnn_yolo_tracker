...
STrack::STrack( std::vector<float> tlwh_, float score,cv::Scalar color)
{
	_tlwh.resize(4);
...
STrack::STrack(std::vector<float> tlwh_, float score, cv::Scalar color,const std::string &class_name){
  _tlwh.resize(4);
  _tlwh.assign(tlwh_.begin(), tlwh_.end());
